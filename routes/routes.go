...
func AllRoutes(app *fiber.App) {
	...
	app.Post(fmt.Sprintf("/api/%s/register", version), middleware.Limiter(5, 40), controller.Register)
	app.Post(fmt.Sprintf("/api/%s/login", version), middleware.Limiter(6, 45), controller.Login)
	app.Post(fmt.Sprintf("/api/%s/resetpassword", version), middleware.Limiter(6, 45), controller.ResetPassword)
	...
	app.Get(fmt.Sprintf("api/%s/getallusers", version), controller.GetAllUsers)
